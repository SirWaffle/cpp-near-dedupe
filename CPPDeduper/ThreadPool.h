...
#define BS_THREAD_POOL_VERSION "v3.3.0 (2022-08-03) Modified"
...
namespace BS
{
    ...
    public:
        ...
        std::future_status wait_for(const std::chrono::duration<_Rep, _Per>& _Rel_time) const
        {
            for (size_t i = 0; i < futures.size(); ++i)
            {
                if (futures[i].wait_for(_Rel_time) != std::future_status::ready)
    ...
    public:
         ...
         */
        thread_pool(concurrency_t minThreadCount, concurrency_t maxThreadCount = 0)
        {
            uint32_t max = GetMaxThreadCount();
            if (maxThreadCount > max || maxThreadCount == 0)
                maxThreadCount = max;
            if (maxThreadCount < minThreadCount)
                maxThreadCount = minThreadCount;

            thread_count = determine_thread_count(maxThreadCount);
            threads = std::make_unique<std::thread[]>(determine_thread_count(maxThreadCount));

            create_threads();
        }

        [[nodiscard]] concurrency_t GetMaxThreadCount()
        {
    ...
    private:
        ...
        mutable std::mutex freecount_mutex = {};
